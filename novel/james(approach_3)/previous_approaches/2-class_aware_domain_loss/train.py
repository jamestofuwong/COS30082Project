    ...
    def forward(self, features, domains, labels):
        ...
        for class_idx in range(self.num_classes):
            ...
            if herbarium_class.size(0) > 1 and photo_class.size(0) > 1:
                ...
                # Add small epsilon to prevent division by zero and NaN
...
class EnhancedHybridModel(nn.Module):
    ...
    def forward(self, resnet_input, dinov2_features, domain_alpha=1.0):
        ...
        return class_logits, domain_logits, fused_features  # Return fused_features for ClassAwareDomainLoss
...
def plot_training_metrics(metrics_file, save_dir):
    ...
    train_accs, val_accs = [], []
    cls_losses, domain_losses, class_aware_losses = [], [], []
...
def train_enhanced_hybrid_model():
    ...
    with open(metrics_file, 'w', newline='') as f:
        ...
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'train_acc', 'val_acc', 
                        'cls_loss', 'domain_loss', 'class_aware_loss', 'paired_acc', 
    ...
    class_aware_criterion = ClassAwareDomainLoss(num_classes=CONFIG['num_classes']).to(device)
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        running_cls_loss, running_domain_loss, running_class_aware_loss = 0.0, 0.0, 0.0
        ...
        for batch_idx, ((features, labels, domains, has_pairs), (imgs, img_labels, _, _)) in enumerate(pbar):
            ...
            # Compute class-aware domain loss
            ...
            # Handle both tensor and float cases for class_aware_loss
            if isinstance(class_aware_loss, torch.Tensor):
                running_class_aware_loss += class_aware_loss.item()
            else:
                running_class_aware_loss += class_aware_loss
            ...
            if batch_idx % 5 == 0:
                pbar.set_postfix({
                    ...
                    'ca_loss': f'{running_class_aware_loss/(batch_idx+1):.4f}',
                    'lr': f'{current_lr:.2e}'
                })
        ...
        train_class_aware_loss = running_class_aware_loss / len(train_loader)
        ...
        with open(metrics_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, train_loss, val_loss, train_acc, val_acc, 
                           train_cls_loss, train_domain_loss, train_class_aware_loss,
