...
CONFIG = {
    ...
    'seed': 42,
    'save_dir': 'testing',
    'contrastive_mode': 'progressive',  # 'fixed', 'progressive', or 'adaptive'
    'contrastive_weights': [0.01, 0.05, 0.1],  # For fixed mode
}

# ==============================================================
# Contrastive Weight Scheduling
# ==============================================================
def get_contrastive_weight(epoch, total_epochs, mode='progressive'):
    """
    Get contrastive loss weight based on epoch and scheduling mode
    """
    if mode == 'fixed':
        # Use medium weight for fixed mode
        return 0.05
    elif mode == 'progressive':
        # Gradual increase as training stabilizes
        if epoch < total_epochs * 0.3:  # First 30%
            return 0.01  # Gentle start
        elif epoch < total_epochs * 0.6:  # Next 30%
            return 0.05  # Medium strength
        else:  # Final 40%
            return 0.1   # Full strength
    elif mode == 'adaptive':
        # More gradual ramp-up
        progress = epoch / total_epochs
        return 0.01 + (0.09 * progress)  # 0.01 to 0.1 linear
    else:
        return 0.05  # Default
...
class DomainContrastiveLoss(nn.Module):
    ...
    def forward(self, features, domains, labels):
        ...
        domains: (batch_size,) - 0 for herbarium, 1 for photo
        ...
        # Create domain masks
        herbarium_mask = (domains == 0).unsqueeze(1)
        photo_mask = (domains == 1).unsqueeze(1)
        
        # Create label mask for positive pairs (same class)
        label_matrix = labels.unsqueeze(1) == labels.unsqueeze(0)
        
        # Cross-domain positive pairs (same class, different domains)
        cross_domain_positives = label_matrix & (herbarium_mask & photo_mask.T)
        
        # Intra-domain positives (same class, same domain) - we want to avoid pulling these too close
        intra_herbarium_positives = label_matrix & herbarium_mask & herbarium_mask.T
        intra_photo_positives = label_matrix & photo_mask & photo_mask.T
        
        # Negative pairs (different classes)
        negatives = ~label_matrix
        
        # Contrastive loss calculation
        losses = []
        
        for i in range(batch_size):
            # Positive pairs: cross-domain same class
            pos_mask = cross_domain_positives[i]
            if pos_mask.sum() > 0:
                pos_similarities = similarity_matrix[i][pos_mask]
            else:
                pos_similarities = torch.tensor([0.0], device=features.device)
            
            # Negative pairs: all different classes
            neg_mask = negatives[i]
            if neg_mask.sum() > 0:
                neg_similarities = similarity_matrix[i][neg_mask]
            else:
                neg_similarities = torch.tensor([0.0], device=features.device)
            
            if len(pos_similarities) > 0 and len(neg_similarities) > 0:
                # InfoNCE-style loss
                pos_term = -torch.log(torch.exp(pos_similarities).sum() + 1e-8)
                neg_term = torch.log(torch.exp(neg_similarities).sum() + 1e-8)
                instance_loss = (pos_term + neg_term) / len(pos_similarities)
            else:
                instance_loss = torch.tensor(0.0, device=features.device)
...
class AdaptiveDomainLoss(nn.Module):
    def __init__(self, alpha=0.3, gamma=2, paired_weight=1.5, contrastive_weight=0.05):
        super().__init__()
        self.focal_loss = FocalLoss(alpha=1, gamma=gamma)
        self.domain_loss = nn.CrossEntropyLoss()
        self.contrastive_loss = DomainContrastiveLoss(temperature=0.1)
        self.alpha = alpha
        self.paired_weight = paired_weight
        self.contrastive_weight = contrastive_weight
        
    def forward(self, class_logits, domain_logits, features, targets, domains, has_pairs, contrastive_weight=None):
        ...
        contrast_loss = self.contrastive_loss(features, domains, targets)
        ...
        total_loss = total_cls_loss + self.alpha * domain_loss + contrastive_weight * contrast_loss
        
        return total_loss, cls_loss.mean(), domain_loss, contrast_loss
...
def plot_training_metrics(metrics_file, save_dir):
    ...
    train_accs, val_accs = [], []
    cls_losses, domain_losses, contrast_losses = [], [], []
    paired_accs, unpaired_accs, minority_accs = [], [], []
    learning_rates, domain_weights = [], []
    ...
    with open(metrics_file, 'r') as f:
        ...
        for row in reader:
            try:
                ...
                contrast_losses.append(float(row.get('contrast_loss', 0.0)))
                ...
                domain_weights.append(float(row.get('domain_weight', 0.0)))
    ...
    ax.plot(epochs, contrast_losses, label='Contrastive Loss')
    ...
    ax = axes[1][2]
    ax.plot(epochs, domain_weights, label='Contrastive Weight', color='red')
    ax.set_title('Contrastive Weight Schedule')
    ax.set_xlabel('Epoch')
...
def train_enhanced_hybrid_model():
    ...
    with open(metrics_file, 'w', newline='') as f:
        ...
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'train_acc', 'val_acc', 
                        'cls_loss', 'domain_loss', 'contrast_loss', 'paired_acc', 
                        'unpaired_acc', 'minority_acc', 'learning_rate', 'domain_weight'])
    ...
    criterion = AdaptiveDomainLoss(alpha=0.3, gamma=2, paired_weight=1.5, contrastive_weight=0.05)
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        contrastive_weight = get_contrastive_weight(
            epoch, CONFIG['epochs'], mode=CONFIG['contrastive_mode']
        )
        
        model.train()
        running_loss, running_correct = 0.0, 0
        running_cls_loss, running_domain_loss, running_contrast_loss = 0.0, 0.0, 0.0
        ...
        for batch_idx, ((features, labels, domains, has_pairs), (imgs, img_labels, _, _)) in enumerate(pbar):
            ...
            class_logits, domain_logits, fused_features = model(imgs, features, domain_alpha=1.0)
            total_loss, cls_loss, domain_loss, contrast_loss = criterion(
                class_logits, domain_logits, fused_features, labels, domains, has_pairs, contrastive_weight
            )
            ...
            if batch_idx % 5 == 0:
                pbar.set_postfix({
                    ...
                    'contrast_w': f'{contrastive_weight:.3f}',
                    'lr': f'{current_lr:.2e}'
                })
        ...
        train_contrast_loss = running_contrast_loss / len(train_loader)
        ...
        with torch.no_grad():
            for (features, labels, domains, has_pairs), (imgs, img_labels, _, _) in val_loader:
                ...
                class_logits, _, _ = model(imgs, features, domain_alpha=0.0)
        ...
        if val_acc > best_val_acc + 0.1:
            ...
            }, os.path.join(CONFIG['save_dir'], "best_enhanced_model.pth"))
        ...
        with open(metrics_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, train_loss, val_loss, train_acc, val_acc, 
                           train_cls_loss, train_domain_loss, train_contrast_loss,
                           paired_acc, unpaired_acc, minority_acc, current_lr, contrastive_weight])
        ...
        print(f"  Contrastive Weight: {contrastive_weight:.3f} | Learning Rate: {current_lr:.2e}")
...
if __name__ == "__main__":
    ...
    print(f"Contrastive Mode: {CONFIG['contrastive_mode']}")
