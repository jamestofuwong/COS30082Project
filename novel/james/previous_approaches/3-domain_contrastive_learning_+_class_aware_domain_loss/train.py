...
CONFIG = {
    ...
    'seed': 42,
    'save_dir': 'testing',
    'use_domain_contrastive': True,      # Toggle domain contrastive learning
    'use_class_aware_domain': True,      # Toggle class-aware domain loss
    'contrastive_weight': 0.1,           # Weight for contrastive loss
    'class_aware_domain_weight': 0.1,    # Weight for class-aware domain loss
...
class ClassAwareDomainLoss(nn.Module):
    ...
    def forward(self, features, domains, labels):
        ...
        for class_idx in range(self.num_classes):
            # Get features for this class from both domains
            class_mask = (labels == class_idx)
            herbarium_class = features[(domains == 0) & class_mask]
            photo_class = features[(domains == 1) & class_mask]
            
            # Check if we have enough samples in both domains
            if herbarium_class.size(0) > 1 and photo_class.size(0) > 1:
                # Center features for this class
                herbarium_centered = herbarium_class - herbarium_class.mean(0, keepdim=True)
                photo_centered = photo_class - photo_class.mean(0, keepdim=True)
                
                # Compute standard deviation with numerical stability
                herbarium_std = herbarium_centered.std(0, unbiased=False) + 1e-8
                photo_std = photo_centered.std(0, unbiased=False) + 1e-8
                
                # Align class-specific distributions with clamping to prevent extreme values
                class_loss = F.mse_loss(
                    torch.clamp(herbarium_std, min=1e-6, max=1e6),
                    torch.clamp(photo_std, min=1e-6, max=1e6)
                )
                
                # Check for NaN or infinite values
                if not torch.isnan(class_loss) and not torch.isinf(class_loss):
                    loss += class_loss
                    valid_classes += 1
        
        # Return average loss only for classes that have valid samples
        if valid_classes > 0:
...
class AdaptiveDomainLoss(nn.Module):
    ...
    def forward(self, class_logits, domain_logits, features, targets, domains, has_pairs):
        ...
        contrastive_loss = torch.tensor(0.0, device=class_logits.device)
        if self.use_contrastive:
            contrastive_loss = self.contrastive_loss(features, domains, targets)
        
        # Paired/unpaired weighting
        ...
        # Total loss
        total_loss = total_cls_loss + self.alpha * domain_loss
        if self.use_contrastive:
...
def plot_training_metrics(metrics_file, save_dir):
    ...
    train_accs, val_accs = [], []
    cls_losses, domain_losses, contrastive_losses, class_aware_losses = [], [], [], []
    ...
    with open(metrics_file, 'r') as f:
        ...
        for row in reader:
            try:
                ...
                class_aware_losses.append(float(row.get('class_aware_loss', 0.0)))
    ...
    if CONFIG['use_domain_contrastive']:
        ax.plot(epochs, contrastive_losses, label='Contrastive Loss')
    if CONFIG['use_class_aware_domain']:
        ax.plot(epochs, class_aware_losses, label='Class Aware Loss')
...
def train_enhanced_hybrid_model():
    ...
    with open(metrics_file, 'w', newline='') as f:
        ...
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'train_acc', 'val_acc', 
                        'cls_loss', 'domain_loss', 'contrastive_loss', 'class_aware_loss',
                        'paired_acc', 'unpaired_acc', 'minority_acc', 'learning_rate'])
    ...
    # Initialize losses based on configuration
    criterion = AdaptiveDomainLoss(
        alpha=0.3, 
        gamma=2, 
        paired_weight=1.5,
        contrastive_weight=CONFIG['contrastive_weight'] if CONFIG['use_domain_contrastive'] else 0.0,
        temperature=CONFIG['temperature'],
        use_contrastive=CONFIG['use_domain_contrastive']
    )
    
    if CONFIG['use_class_aware_domain']:
        class_aware_criterion = ClassAwareDomainLoss(num_classes=simple_ds.num_classes).to(device)
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        running_cls_loss, running_domain_loss, running_contrastive_loss, running_class_aware_loss = 0.0, 0.0, 0.0, 0.0
        ...
        for batch_idx, ((features, labels, domains, has_pairs), (imgs, img_labels, _, _)) in enumerate(pbar):
            ...
            # Get features for contrastive learning and class-aware domain loss
            ...
            # Add class-aware domain loss if enabled
            if CONFIG['use_class_aware_domain']:
                class_aware_loss = class_aware_criterion(fused_features, domains, labels)
                total_loss = total_loss + CONFIG['class_aware_domain_weight'] * class_aware_loss
                running_class_aware_loss += class_aware_loss.item() if isinstance(class_aware_loss, torch.Tensor) else class_aware_loss
            ...
            if batch_idx % 5 == 0:
                ...
                if CONFIG['use_domain_contrastive']:
                    postfix_dict['cont'] = f'{running_contrastive_loss/(batch_idx+1):.4f}'
                if CONFIG['use_class_aware_domain']:
                    postfix_dict['ca_loss'] = f'{running_class_aware_loss/(batch_idx+1):.4f}'
        ...
        train_class_aware_loss = running_class_aware_loss / len(train_loader) if CONFIG['use_class_aware_domain'] else 0.0
        ...
        with open(metrics_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, train_loss, val_loss, train_acc, val_acc, 
                           train_cls_loss, train_domain_loss, train_contrastive_loss, train_class_aware_loss,
                           paired_acc, unpaired_acc, minority_acc, current_lr])
        
        print(f"\nEpoch {epoch}:")
        print(f"  Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")
        print(f"  Train Acc: {train_acc:.2f}% | Val Acc: {val_acc:.2f}%")
        print(f"  Cls Loss: {train_cls_loss:.4f} | Domain Loss: {train_domain_loss:.4f}")
        if CONFIG['use_domain_contrastive']:
            print(f"  Contrastive Loss: {train_contrastive_loss:.4f}")
        if CONFIG['use_class_aware_domain']:
            print(f"  Class Aware Loss: {train_class_aware_loss:.4f}")
...
if __name__ == "__main__":
    ...
    print(f"Using Domain Contrastive Learning: {CONFIG['use_domain_contrastive']}")
    print(f"Using Class-Aware Domain Loss: {CONFIG['use_class_aware_domain']}")
